  ...
  return (
    <div className="report">
      <div className="icon-wrapper">
        <img alt="icon" className="report-icon" src={getIcon(props.data[props.data.length - 1]) ?? ''} />
      </div>
      <div>
        <WeatherMetric
          title={'Temperature'}
        ...
        <WeatherMetric
          title={'Cloud Cover'}
          data={props.data}
          getMetric={d => d.cloud_cover_total ?? 0}
          unit="%"
          view={{ dataMin: 0, dataMax: 100 }}
        />
        <WeatherMetric
          title={'Humidity'}
          data={props.data}
          getMetric={d => d.relative_humidity ?? 0}
          unit="%"
          view={{ dataMin: 0, dataMax: 100 }}
        />
      </div>
...
function pathFromData(
  images: Record<'day' | 'night', Record<number, string>>,
  data: ReportDataEl[],
): string | undefined {
